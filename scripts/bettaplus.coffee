...
class ScoreKeeper
  ...
  removeUser: (user) ->
    delete @robot.brain.data.scores[user]
...
module.exports = (robot) ->
  ...
  robot.hear /^(top|bottom) (\d+)/i, (msg) ->
    ...
    for i in [0..tops.length-1]
      if /^@/.test(tops[i].name)
        fixedname = tops[i].name.replace '@', ''
        _(tops[i].score).times ->
          scoreKeeper.subtract(tops[i].name, 'ginger-llama')
          scoreKeeper.add(fixedname, 'ginger-llama')
        scoreKeeper.removeUser(tops[i].name)
      message.push("#{i+1}. #{tops[i].name} : #{tops[i].score}")

    msg.send message.join("\n")

  # robot.hear /^unscore (.+)/, (msg) ->
  #   scoreKeeper.removeUser(msg.match[1])
  #   msg.send 'Removed score for ' + msg.match[1]
